...
const Events = () => {
    return (
        <section className="xl:pb-23 xl:pt-38 md:pb-16 lg:pt-24 pb-10 md:pt-16 pt-10 px-4">
                    ...
                    <div className="flex flex-wrap gap-6 justify-center">
                        {eventCards.map((event, index) => (
                            <div key={index} className="w-full md:w-[48%] xl:w-[38%] group cursor-pointer">
                                <div className="transition-all duration-300 group-hover:shadow-md rounded overflow-hidden border border-none group-hover:bg-white">
                                    <img
                                        src={event.img}
                                        alt={event.alt}
                    ...
                    <div className="w-full lg:w-2/5">
                        ...
                        <form className="space-y-4 mt-4">
                            {inputGroups.map((group, groupIndex) => (
                                <div key={groupIndex} className={`flex ${group.length > 1 ? 'flex-row gap-4' : 'flex-col'}`}>
                                    {group.map((field, fieldIndex) => (
                                        <input
                                            key={fieldIndex}
                                            type={field.type}
                                            placeholder={field.placeholder}
                                            className="w-full p-2 border border-[#00000066] rounded font-normal text-base"
                                        />
                                    ))}
                                </div>
                            ))}
                            <textarea
                                ...
                                className="w-full p-2 border border-[#00000066] rounded h-24 resize-none font-normal text-base"
                            ></textarea>
